...
const updatedFunctions = `
  const handleFinalAbandonConfirm = async () => {
      ...
      const reqAbandon = {
          ...
          target_colLabel: columnConfigs.find((c: any) => c.column_id === replacementCell.col)?.custom_label || columnConfigs.find((c: any) => c.column_id === replacementCell.col)?.name,
          status: 'APPROVED',
